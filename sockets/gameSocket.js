  ...
  io.on("connection", (socket) => {
    ...
    socket.on("joinRoom", async (data) => {
      try {
        ...
        // Emit room joined confirmation to the joining player
        ...
        // Also emit player-update for game room page compatibility
    ...
    // Handle rejoin game (for game room page)
    socket.on("rejoin-game", async (data) => {
      try {
        ...
        console.log(`🔄 Player ${playerName} rejoined room ${gameCode}`);

        // Send current room state to rejoining player
        socket.emit("player-update", gameRoom.players);

        // Notify room of player reconnection
        io.to(gameCode).emit("player-update", gameRoom.players);
      } catch (error) {
        console.error("Rejoin game error:", error);
        socket.emit("game-error", "Failed to rejoin game");
      }
    });
    ...
    socket.on("submitPrediction", async (data) => {
      try {
        ...
        console.log(
          `📝 Player ${playerId} submitted prediction: ${predictedChoice}`
        );
...
async function startRound(gameCode, io) {
  try {
    ...
    console.log(
      `🎯 Starting round ${gameRoom.currentRound} for room ${gameCode}`
    );
...
async function calculateScoresAndProceed(gameCode, io) {
  try {
    ...
    console.log(
      `📊 Calculating scores for round ${gameRoom.currentRound} in room ${gameCode}`
    );
    ...
    // Wait for EXACTLY 3 seconds score display time, then proceed to next round or end game
    const scoreDisplayTime = 3; // Fixed at exactly 3 seconds as per requirements
    ...
    const scoreTimer = setTimeout(async () => {
      await proceedToNextRoundOrEndGame(gameCode, io);
    }, scoreDisplayTime * 1000); // Exactly 3 seconds

    gameTimers.get(gameCode).scoreTimer = scoreTimer;
  } catch (error) {
...
async function proceedToNextRoundOrEndGame(gameCode, io) {
  try {
    ...
    console.log(
      `🎮 Round ${gameRoom.currentRound} completed for room ${gameCode}`
    );
...
async function endGame(gameCode, io) {
  try {
    ...
    io.to(gameCode).emit("finalScores", {
      ...
      gameStats: {
        ...
        averageScore: Math.round(
          gameRoom.leaderboard.reduce((sum, p) => sum + p.score, 0) /
            gameRoom.leaderboard.length
        ),
        topScore: gameRoom.leaderboard[0]?.score || 0,
      },
      displayTime: 10, // Fixed at exactly 10 seconds as per requirements
    });

    // Show winner animation after EXACTLY 10 seconds of final score display
    const finalScoreDisplayTime = 10; // Fixed at exactly 10 seconds
    ...
    const finalScoreTimer = setTimeout(async () => {
      await showWinnerAnimation(gameCode, io);
    }, finalScoreDisplayTime * 1000); // Exactly 10 seconds

    gameTimers.get(gameCode).finalScoreTimer = finalScoreTimer;
  } catch (error) {
...
async function showWinnerAnimation(gameCode, io) {
  try {
    ...
    console.log(
      `🏆 Showing winner animation for ${winner.playerName} in room ${gameCode}`
    );
    ...
    io.to(gameCode).emit("winnerAnimation", {
      ...
      celebration: {
        ...
        duration: 10, // Fixed at exactly 10 seconds as per requirements
      },
      finalLeaderboard: gameRoom.leaderboard,
    });

    // Complete game after EXACTLY 10 seconds of winner animation
    const winnerAnimationTime = 10; // Fixed at exactly 10 seconds
    ...
    const winnerAnimationTimer = setTimeout(async () => {
      await completeGame(gameCode, io);
    }, winnerAnimationTime * 1000); // Exactly 10 seconds

    gameTimers.get(gameCode).winnerAnimationTimer = winnerAnimationTimer;
  } catch (error) {
